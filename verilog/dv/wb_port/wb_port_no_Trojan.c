...
void main()
{
	...
	*/
	...
	// Flag register init done
	reg_mprj_datal = 0xFFF00000;

	reg_la0_data = 0x00000004; //start=1,decrypt=0,Mux=DES(0)
	reg_la0_data = 0x00000000; //start=0
	
	// Flag la start Set 1 and 0
	reg_mprj_datal = 0xF0F00000;
	
	reg_mprj_datal = reg_la0_data_in;
	reg_mprj_datal = 0xF1F00000;
	
	while ((reg_la0_data_in && 0x00000008)  != 1); //Wait for the finish signal
	reg_mprj_datal=reg_la0_data_in;
	reg_mprj_datal = 0xff100000;

	 
	while ((o_Data_ADDR_0 != 0x150E2451)); //Wait for the finish signal
	reg_mprj_datal = 0xff200000;
	while ((o_Data_ADDR_1 != 0x355550B2)); //Wait for the finish signal
	reg_mprj_datal = 0xff300000;
	...
	reg_mprj_datal = 0xff400000;

	while ((o_Data_ADDR_0 != 0xdb0909bc)); //Wait for the finish signal
	reg_mprj_datal = 0xff500000;
	while ((o_Data_ADDR_1 != 0x4e615668)); //Wait for the finish signal
	reg_mprj_datal = 0xff600000;
	while ((o_Data_ADDR_2 != 0x7e59fa9e)); //Wait for the finish signal
	reg_mprj_datal = 0xff700000;
	while ((o_Data_ADDR_3 != 0x747cb926)); //Wait for the finish signal
	reg_mprj_datal = 0xff800000;
}
